...
Collectively, PaperBanana paves the way for the automated generation of publication-ready illustrations.Google Confidential & ProprietaryGoogle Confidential & Proprietary
...
arXiv:2601.23265v1 [cs.CL] 30 Jan 2026PaperBanana: Automating Academic Illustration for AI Scientists
...
2PaperBanana: Automating Academic Illustration for AI Scientists
...
source context and a communicative intent to a visual representation. Let 𝑆 denote the source context
containing the essential information, and 𝐶 denote the communicative intent that specifies the scope
and focus of the desired illustration. The goal is to generate an image 𝐼 that faithfully visualizes 𝑆
while fulfilling the communicative intent 𝐶, formulated as:
𝐼 = 𝑓 (𝑆, 𝐶). (1)
To further guide the mapping function, the input can be optionally augmented by a set of 𝑁 reference
examples E = {𝐸𝑛 }𝑁
𝑛=1. Each example 𝐸𝑛 serves as a ground-truth demonstration, defined as a
tuple 𝐸𝑛 = (𝑆𝑛, 𝐶𝑛, 𝐼𝑛), where 𝐼𝑛 is the reference illustration corresponding to the context 𝑆𝑛 and
...
the source context 𝑆 is the textual description of the method (e.g., methodology sections), and the
communicative intent 𝐶 is the figure caption specifying the scope and focus (e.g., “Overview of our
framework”).
3. Methodology
...
Retriever Agent. Given the source context 𝑆 and the communicative intent 𝐶, the Retriever Agent
identifies 𝑁 most relevant examples E = {𝐸𝑛 }𝑁
𝑛=1 ⊂ R from the fixed reference set R to guide the
downstream agents. As defined in Section 2, each example 𝐸𝑖 ∈ R is a triplet (𝑆𝑖, 𝐶𝑖, 𝐼𝑖). To leverage
...
over topic similarity. By explicitly reasoned selection of reference illustrations 𝐼𝑖 whose corresponding
3PaperBanana: Automating Academic Illustration for AI Scientists
...
context 𝑆, communicative intent 𝐶, and retrieved examples E as inputs. By performing in-context
...
implementation details). Armed with this guideline, the Stylist refines each initial description 𝑃 into
...
Visualizer Agent. After receiving the stylistically optimized description 𝑃∗, the Visualizer Agent
...
𝐼𝑡 = Image-Gen(𝑃𝑡 ), (6)
where the initial description 𝑃0 is set to 𝑃∗.
4PaperBanana: Automating Academic Illustration for AI Scientists
Critic Agent. The Critic Agent forms a closed-loop refinement mechanism with the Visualizer by
closely examining the generated image 𝐼𝑡 and providing refined description 𝑃𝑡+1 to the Visualizer.
Upon receiving the generated image 𝐼𝑡 at iteration 𝑡, the Critic inspects it against the original source
context (𝑆, 𝐶) to identify factual misalignments, visual glitches, or areas for improvement. It then
provides targeted feedback and produces a refined description 𝑃𝑡+1 that addresses the identified
issues:
...
iterates for 𝑇 = 3 rounds, with the final output being 𝐼 = 𝐼𝑇 . This iterative refinement process ensures
...
and Critic agents. For numerical precision, the Visualizer converts the description 𝑃𝑡 into executable
Python Matplotlib code: 𝐼𝑡 = VLMcode (𝑃𝑡 ). The Critic evaluates the rendered plot and generates a
refined description 𝑃𝑡+1 addressing inaccuracies or imperfections: 𝑃𝑡+1 = VLMcritic (𝐼𝑡 , 𝑆, 𝐶, 𝑃𝑡 ). The
same 𝑇 = 3 round iterative refinement process applies. While we prioritize this code-based approach
...
each a tuple (𝑆, 𝐼, 𝐶), where 𝑆 is the methodology description, 𝐼 is the methodology diagram,
and 𝐶 is the caption.
Categorization. To facilitate future analysis of generating different types of diagrams, we further
5PaperBanana: Automating Academic Illustration for AI Scientists
...
• Content (Faithfulness & Conciseness): Faithfulness ensures alignment with the source context
...
• Presentation (Readability & Aesthetics): Readability demands intelligible layouts, legible text,
...
6PaperBanana: Automating Academic Illustration for AI Scientists
...
7PaperBanana: Automating Academic Illustration for AI Scientists
...
8PaperBanana: Automating Academic Illustration for AI Scientists
...
9PaperBanana: Automating Academic Illustration for AI Scientists
...
10PaperBanana: Automating Academic Illustration for AI Scientists
...
11PaperBanana: Automating Academic Illustration for AI Scientists
...
12PaperBanana: Automating Academic Illustration for AI Scientists
Impact Statement
...
Conference on Computer Vision, pages 17793–17806, 2025.
...
V. Dibia and Ç. Demiralp. Data2vis: Automatic generation of data visualizations using sequenceto-sequence recurrent neural networks. IEEE computer graphics and applications, 39(5):33–46,
2019.
13PaperBanana: Automating Academic Illustration for AI Scientists
...
14PaperBanana: Automating Academic Illustration for AI Scientists
...
15PaperBanana: Automating Academic Illustration for AI Scientists
...
Association for Computational Linguistics ACL 2024, pages 11789–11804, 2024.
...
16PaperBanana: Automating Academic Illustration for AI Scientists
...
17PaperBanana: Automating Academic Illustration for AI Scientists
...
stylistic improvements in color schemes, typography, graphical elements, etc.Original Diagrams Style Enhanced Diagrams
...
18PaperBanana: Automating Academic Illustration for AI ScientistsPlots Visualized via IMG Plots Visualized via Coding Case Analysis
...
19PaperBanana: Automating Academic Illustration for AI Scientists
...
foundation model.Diagrams from PaperBanana Failure Reasons
The attention block incorrectly draws
connections from the Noise Predictor (
) to three destination boxes,𝑓𝑠𝑑
labeling two of them as 'Keys ( )'𝐾𝑖
and one as 'Values ( )'. The text (Eq.𝑉𝑖
2) explicitly states that diffusion
features map only to Keys and Values,
while Queries ( ) come from𝑄𝑖
learnable parameters.
It fails to draw generative connections
(solid lines) from Actions ( ) to𝑎
Task-Relevant States ( ). The method𝑠
explicitly states (Eq. 3) that state
transitions depend on previous actions
( ). By isolating theℎ'𝑡 = 𝑓(..., 𝑎𝑡−1)
...
20PaperBanana: Automating Academic Illustration for AI Scientists
...
21PaperBanana: Automating Academic Illustration for AI Scientists
...
22PaperBanana: Automating Academic Illustration for AI Scientists
...
23PaperBanana: Automating Academic Illustration for AI Scientists
...
Description ($P^*)"**.
24PaperBanana: Automating Academic Illustration for AI Scientists
...
25PaperBanana: Automating Academic Illustration for AI Scientists
#### **A. Color Palettes**
...
26PaperBanana: Automating Academic Illustration for AI Scientists
#### **C. Lines & Arrows**
...
Gradient Flow."
27PaperBanana: Automating Academic Illustration for AI Scientists
...
grey, and navy.
28PaperBanana: Automating Academic Illustration for AI Scientists
...
29PaperBanana: Automating Academic Illustration for AI Scientists
...
30PaperBanana: Automating Academic Illustration for AI Scientists
...
in this batch.
31PaperBanana: Automating Academic Illustration for AI Scientists
...
32PaperBanana: Automating Academic Illustration for AI Scientists
...
Below are multiple visual analysis reports from a dataset of NeurIPS 2025
statistical plots.
33PaperBanana: Automating Academic Illustration for AI Scientists
...
34PaperBanana: Automating Academic Illustration for AI Scientists
target paper.
# Your Task
...
35PaperBanana: Automating Academic Illustration for AI Scientists
Candidate Paper i:
...
## ROLE
36PaperBanana: Automating Academic Illustration for AI Scientists
...
37PaperBanana: Automating Academic Illustration for AI Scientists
...
38PaperBanana: Automating Academic Illustration for AI Scientists
## INPUT DATA
...
39PaperBanana: Automating Academic Illustration for AI Scientists
...
"ref_100",
"ref_42",
40PaperBanana: Automating Academic Illustration for AI Scientists
"ref_7",
"ref_156",
"ref_89",
"ref_3",
...
**Crucial Instructions:**
41PaperBanana: Automating Academic Illustration for AI Scientists
...
42PaperBanana: Automating Academic Illustration for AI Scientists
...
‘‘‘
43PaperBanana: Automating Academic Illustration for AI Scientists
...
44PaperBanana: Automating Academic Illustration for AI Scientists
...
test immediately:**
45PaperBanana: Automating Academic Illustration for AI Scientists
...
Diagram**.
# Inputs
46PaperBanana: Automating Academic Illustration for AI Scientists
...
47PaperBanana: Automating Academic Illustration for AI Scientists
...
48PaperBanana: Automating Academic Illustration for AI Scientists
